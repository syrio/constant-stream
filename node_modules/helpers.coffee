...
class SessionManager extends EventEmitter
  ...
  handleDisconnect: (session) =>
    ...
    # in order to save the need for keyname parsing later on,
    # set session.id as value even though the keyname already contains it
    @storage.store("offline:#{session.id}:at:#{minuteCode()}", session.id)
  ...
  handleReconnect: (session) =>
    ...
    @storage.keys "offline:#{session.id}:at:*", (err, timestamps) =>
  ...
  handleHeartbeat: (checked_period) =>
    ...
    sessions_within_checked_period = "offline:*:at:#{checked_period}*"
